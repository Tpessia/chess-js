...
const _chessCoordinateToMatrixMap: StrictEnumDictionary<ChessCoordinate, ChessCoordinateMatrix> = _chessCoordinateList.reduce((acc, val, i) => ({ ...acc, [val]: { coordinate: val, rowIndex: Math.floor(i / 8), colIndex: i % 8 } as ChessCoordinateMatrix }), {} as any);
const _chessCoordinateFromMatrixMap: NumberDictionary<NumberDictionary<ChessCoordinateMatrix>> = _chessCoordinateList.reduce((acc, val, i) => {
    const colIndex = i % 8;
    const rowIndex = Math.floor(i / 8);
    if (acc[rowIndex] == null) acc[rowIndex] = {};
    acc[rowIndex]![colIndex] = { coordinate: val, rowIndex, colIndex };
    return acc;
